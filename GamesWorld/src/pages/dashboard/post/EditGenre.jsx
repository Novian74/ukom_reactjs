...
export default function EditGenre() {
  ...
  const fetchApi = async () => {
    await Api.get("/api/kategori/" + id.id).then((response) => {
      let data = response.data;
      data.map((item) => {
        setGenre(item.kategori);
      });
    });
  };

  const EditGenreGame = async (e) => {
    e.preventDefault();
    ...
    Api.post("api/kategori/update/" + id.id, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });

    swal("Success!", "Berhasil Ubah Genre!", "success");
        ...
        <div id="content">
          ...
          <div className="container-fluid">
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
              <h1 className="h3 mb-0 text-light">Edit Genre</h1>
            </div>

            <form onSubmit={EditGenreGame} className="text-white">
              <hr className="mt-5" />
              {/* GENRE NAME */}
